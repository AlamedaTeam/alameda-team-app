...
export default function Signup() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // 2) (opcional/backup) crear perfil si el trigger no lo hace
    ...
    <div className="relative z-20 flex flex-col items-center justify-center min-h-screen px-4 text-white">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4 w-full max-w-xs">
        <input
          ...
          className="bg-black/50 text-white placeholder-white rounded-xl px-4 py-3 focus:outline-none"
          value={email}
        ...
        <button
          type="submit"
          disabled={loading}
          className="bg-white text-black font-semibold py-3 rounded-xl shadow-md disabled:opacity-60"
        >
      ...
      <div className="flex flex-col space-y-3 mt-6 w-full max-w-xs text-center">
        <a href="/login" className="text-white underline">Ya tengo cuenta</a>
        <a href="/" className="text-white underline">Volver</a>
      </div>
    </div>
  )
}
